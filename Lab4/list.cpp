...
Phone_node* List::search(char *target){
	...
	while (traverse){
		...
		if (all){
			...
			cout << "Name: " << traverse->name << endl;                       // searchedlist(traverse->name, traverse->lastname, traverse->phonenum);
...
int List::search2(char *target, char *targetlastname){   // for searching newname is exist or not before. 
	Phone_node *traverse;
	int counter = 0;
	int found = 0;
	traverse = head;
	...
	while (traverse){
		counter++;
		if (strncmp(target, traverse->name, strlen(traverse->name)) == 0 && strncmp(targetlastname, traverse->lastname, strlen(traverse->lastname))){  //isim ayn˝ysa yeni ezgiyi yazd˝rm˝yor
			found++;
		}
		traverse = traverse->next;
	}
...
void List::update(Phone_node*found, char *newname, char *newlastname){
	Phone_node *traverse;
	int counter = 1;
	traverse = head;
	...
	while (traverse){
		if (strcmp(traverse->name, found->name) == 0){
			break;
		}
		traverse = traverse->next;
	}
