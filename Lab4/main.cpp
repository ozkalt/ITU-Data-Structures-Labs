...
void add_record(){
	char name[NAME_LENGTH], lastname[NAME_LENGTH], phonenum[PHONENUM_LENGTH];
	...
	cout << "Phone Number: ";
	cin.getline(phonenum, PHONENUM_LENGTH); 
	int validity = book.search2(name, lastname);                  // for searching newname is exist or not before. 
	if (validity != 0){
		cout << "This name already exists!" << endl;
		return;
	}
	book.insert(name, lastname, phonenum);
...
void update_record(){
	...
	Phone_node *found = book.search(name);
	...
	if (found == NULL)
		...
		book.update(found, updatename, updatelastname);
